    ...
    if (code.startsWith(productGen.generation_id)) {
      ...
      if (extractdLevel == 0) {
        ...
        for (let i = 1; i < batch.productHistory.packagingHierarchy; i++) {
          ...
          const updateRecord = await prisma.$queryRawUnsafe(`UPDATE "${productGen.generation_id.toLowerCase()}${currentLevel}_codes" SET parent_id = NULL,is_aggregated=false WHERE id = CAST($1 AS UUID) RETURNING id`, id);
